...
uv run swebench-eval "./eval_outputs_fuzz_hypo_final_aug/${INSTANCES_SUBDIR}/${DATASET_PATH}/${MODEL_PATH_SUFFIX}/output.jsonl" \
  --dataset princeton-nlp/SWE-bench_Verified \
  --output-file results_fuzz_hypo_final_aug_${INSTANCES_SUBDIR}_${MODEL_SAFE_SUFFIX}.swebench.jsonl \
  --workers 5
...
uv run swebench-eval "./eval_outputs_aug/${INSTANCES_SUBDIR}/${DATASET_PATH}/${MODEL_PATH_SUFFIX}/output.jsonl" \
  --dataset princeton-nlp/SWE-bench_Verified \
  --output-file results_aug_${INSTANCES_SUBDIR}_${MODEL_SAFE_SUFFIX}.swebench.jsonl \
  --workers 5

uv run swebench-eval "./eval_outputs_aug/instances_hard/princeton-nlp__SWE-bench_Verified-test/openrouter/anthropic/claude-sonnet-4_sdk_73769d5_maxiter_200_N_initial/output.jsonl" \
  --dataset princeton-nlp/SWE-bench_Verified \
  --output-file results_aug_instances_hard_claude-sonnet-4_sdk_73769d5_maxiter_200_N_initial.swebench.jsonl \
  --workers 5